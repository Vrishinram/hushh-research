...
function getViewCount(center: ConsentCenterResponse | null, actor: ConsentCenterActor, surfaceView: ConsentSurfaceView) {
  if (!center) return 0;
...
export default function ConsentsPage() {
  ...
  const { personaState, riaCapability } = usePersonaState();
  const fallbackActor: ConsentCenterActor =
    personaState?.active_persona === "ria" || personaState?.last_active_persona === "ria"
      ? "ria"
      : "investor";
  const requestedActor = normalizeActor(searchParams.get("actor"), fallbackActor);
  const actor = requestedActor === "ria" && riaCapability !== "switch" ? fallbackActor : requestedActor;
  const surfaceView = normalizeSurfaceView(searchParams.get("view"));
  ...
  const actorOptions = useMemo<SegmentedPillOption[]>(() => {
    const canUseRia = riaCapability !== "disabled";
    return [
      { value: "investor", label: "Investor", icon: UserRound },
      {
        value: "ria",
        label: riaCapability === "setup" ? "Set up RIA" : "RIA",
        icon: BriefcaseBusiness,
        tone: canUseRia ? "accent" : "default",
        disabled: !canUseRia,
      },
    ];
  }, [riaCapability]);
  ...
  function updateSearch(nextActor: ConsentCenterActor, nextView: ConsentSurfaceView) {
    const params = new URLSearchParams(searchParams.toString());
    params.set("actor", nextActor);
    params.set("view", resolveRequestView(nextActor, nextView));
  ...
  return (
    <div className="mx-auto w-full max-w-4xl space-y-6 px-4 py-6 sm:px-6 md:py-8">
      <PageHeader
      ...
      <div className="grid gap-4 lg:grid-cols-[minmax(0,220px)_minmax(0,1fr)] lg:items-end">
        <div className="space-y-2">
          ...
          <SegmentedPill
            ...
            onValueChange={(next) => {
              if (next === "ria" && riaCapability === "setup") {
                router.push(ROUTES.RIA_ONBOARDING);
                return;
              }
              if (next === "ria" && riaCapability !== "switch") {
                return;
              }
              updateSearch(next as ConsentCenterActor, surfaceView);
            }}
            ariaLabel="Consent center persona"
            className="w-full"
          />
        </div>
        ...
        <div className="space-y-2">
          ...
          <SegmentedPill
            ...
            value={surfaceView}
            options={viewOptions}
            onValueChange={(next) => updateSearch(actor, next as ConsentSurfaceView)}
      ...
      <div className="space-y-3">
        ...
        <ContentSurface className="p-0">
          ...
          {!loading && !error && visibleEntries.length === 0 ? (
            <div className="px-5 py-8 text-sm text-muted-foreground">{emptyStateCopy(actor, surfaceView)}</div>
          ) : null}
                    ...
                    <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                      <div className="min-w-0 space-y-2">
                        <div className="flex flex-wrap items-center gap-2">
                          ...
                          <Badge className={statusTone(entry.status)}>{entry.status.replace(/_/g, " ")}</Badge>
                        ...
                        <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-muted-foreground">
                          ...
                          {formatDate(entry.issued_at) ? <span>Issued: {formatDate(entry.issued_at)}</span> : null}
                          {formatDate(entry.expires_at) ? <span>Expires: {formatDate(entry.expires_at)}</span> : null}
                          {entry.relationship_status ? <span>Relationship: {entry.relationship_status}</span> : null}
                        </div>
                      </div>
